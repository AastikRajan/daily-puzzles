...
interface ProgressState {
  ...
  hintDate: string;
  hintsUsed: number;
  recordCompletion: (date: string, type: PuzzleType, c: Completion) => void;
  /** Returns true if a hint was available (and consumes it). */
  consumeHint: () => boolean;
  hintsLeft: () => number;
}
...
export function isCompleted(log: CompletionLog, date: string, type: PuzzleType): boolean {
