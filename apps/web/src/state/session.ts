...
/**
 * One puzzle-playing session, persisted to localStorage so the player can
 * leave and resume. Undo/redo over full state snapshots (grids are tiny).
 * The timer ticks only while the tab is visible and the puzzle is unsolved.
 */
export function usePuzzleSession<TState>(opts: {
  ...
  // persist (cheap — grids are tiny), and skip the very first render
